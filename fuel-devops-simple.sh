...
setupFuelDevops() {
  ...
  pip install git+https://github.com/openstack/fuel-devops.git@2.9.20 \
    --upgrade
}
...
validateAndSetOptions() {
  ...
  if [ "$database" != "postgres" ] && [ "$database" != "sqlite" ]; then
    ...
    exit 1
  elif [ "$fuelVersion" != "7.0" ] && [ "$fuelVersion" != "8.0" ]; then
    usage
